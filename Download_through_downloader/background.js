...
function xhr_call_inpage_notyf__iggwrurefj(url, loadend) {
...
function xhr_call_inpage_result__iggwrurefj(event, notyf, requestedUrl, link, texts) {
	var linkify = function(text) {
		return '<a href="' + link + '" target="_blank">' + text + '</a>';
	};
	if (!event.target) {
		notyf.alert(linkify(texts['failed'] + requestedUrl));
	} else if (event.target.readyState === 4) {
		try {
			var response = JSON.parse(event.target.responseText);
			if (response.added > 0) {
				for (var c = 0; c < response.addedTitles.length; c++) {
					notyf.confirm(linkify(texts['success'] + response.addedTitles[c]));
				}
			}
			if (response.skipped > 0) {
				for (var d = 0; d < response.skippedUrls.length; d++) {
					notyf.confirm(linkify(texts['skipped'] + response.skippedUrls[d]));
				}
			}
			if (response.errors > 0) {
				for (var f = 0; f < response.errorsUrls.length; f++) {
					notyf.alert(linkify(texts['error'] + response.errorsUrls[f]));
				}
			}
		} catch (error) {
			notyf.alert(linkify(texts['failed'] + requestedUrl));
...
if (typeof browser !== 'undefined') {
	...
	function osm_piskvor_org_share(e) {
		...
		browser.tabs.executeScript({
			code: "!function(){ " + notyf_setup_inpage__iggwrurefj.toString() + ";" + xhr_call_inpage_notyf__iggwrurefj.toString() + ";" + xhr_call_inpage_result__iggwrurefj.toString() + /* define the functions in page... */
			";var notyf=notyf_setup_inpage__iggwrurefj(); xhr_call_inpage_notyf__iggwrurefj('" + loadUrl + "',function(e){xhr_call_inpage_result__iggwrurefj(e, notyf, '" + url + "','" + downloaderLink + "'," + JSON.stringify(texts) + ")})}();" /* ...and call them */
		});
	}
